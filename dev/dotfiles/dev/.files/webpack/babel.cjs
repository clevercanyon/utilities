...
module.exports = ( srcDir = '' ) => {
	...
	// Establish compilation targeting for this src directory.

	let srcDirTarget                   = [ 'browserslist:any' ];
	let srcDirTargetHasBrowserslist    = false; // Initalize browserslist detection.
	let srcDirTargetHasBrowserslistEnv = undefined; // Initalize browserslist environment detection.

	for ( const [ fileName, fileData ] of Object.entries( customDataByFile ) ) {
		switch ( fileName ) {
			case 'package.json':
				srcDirTarget = fileData?.config?.c10n?.[ '&' ]?.webpack?.config?.target || srcDirTarget;
				break;
		}
	}
	for ( const [ fileName, fileData ] of Object.entries( customDataByFile ) ) {
		switch ( fileName ) {
			case 'package.json':
				srcDirTarget = fileData?.config?.c10n?.[ '&' ]?.[ 'webpack:' + srcDir ]?.config?.target || srcDirTarget;
				break;
		}
	}
	srcDirTarget.forEach( ( target ) => {
		if ( 'browserslist' === target || /^browserslist:./u.test( target ) ) {
			srcDirTargetHasBrowserslist = true;
		}
		if ( 'browserslist' === target ) {
			srcDirTargetHasBrowserslistEnv = 'any';
		} else if ( /^browserslist:./u.test( target ) ) {
			srcDirTargetHasBrowserslistEnv = target.replace( /^browserslist:/u, '' );
		}
	} );
	...
	const srcDirConfig = {
		root                   : absSrcDir,
		browserslistConfigFile : srcDirTargetHasBrowserslist,
		browserslistEnv        : srcDirTargetHasBrowserslistEnv,
		sourceType             : 'module' === customDataByFile[ 'package.json' ].type ? 'module' : 'script',
		presets                : [
			[ '@babel/preset-env', {} ],
			[ '@babel/preset-react', {} ],
			[ '@babel/preset-typescript', {} ],
