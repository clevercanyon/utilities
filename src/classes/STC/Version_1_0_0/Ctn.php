 ...
 * @since 2021-12-15
 */
class Ctn extends \Clever_Canyon\Utilities\STC\Version_1_0_0\Abstracts\A6t_Stc_Base {
	/**
	 * A collection?
	 *
	 * @since 2021-12-16
	 ...
	 *                             and it offers more granular control over how deep cloning is conducted — see code below.
	 *
	...
	public static function clone_deep( /* object|array */ $ctn, /* string|null */ ?string $method = null ) /* : object|array */ {
		...
		switch ( $method ) {
			case 'json':
				return json_decode( json_encode( $ctn ) ); // JSON approach helpful in some cases.

			case 'serialize':
				return unserialize( serialize( $ctn ) ); // phpcs:ignore -- ☜(▀̿ ͜▀̿ ̿) ok, see notes above.

			case 'reflection':
			default: // `reflection` is the default approach.
				if ( is_object( $ctn ) ) {
	...
	protected static function clone_deep_obj_helper( object $obj, \SplObjectStorage $map ) : object {
		...
		foreach ( $obj_clone_r8n->getProperties() as $_prop ) {
			...
			} // Currently no solution for protected/private references.
	 ...
	 *
	 * @return object|array {@see merge()} for details.
	 */
	 ...
	 *                      Numerically indexed arrays will always replace the original arrays entirely.
	 *
	 * @note  Declarative operations are supported by this variant of {@see merge()}.
	 ...
	 * @param object|null      $_r   Internal use only — do not pass.
	 *
	 * @return object|array {@see super_merge()} for details.
	 */
	 ...
	 *
	 * @return bool True if OP carried out. {@see super_merge_helper()} for further details.
	 */
