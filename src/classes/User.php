 ...
 * @since 2021-12-15
 */
final class User extends U\A6t\Stc_Utilities {
	/**
	 * Traits.
	 *
	 * @since 2021-12-15
	 */
	use U\Traits\User\Members;
	...
	public static function ip() : string {
		...
		foreach ( $env_vars as $_var ) {
			...
			if ( $_ips && ( $_ip = U\User::ip_user_public_helper( $_ips ) ) ) {
				return $_ip; // IPv4 or IPv6 address.
			}
		}
		return ''; // Nothing.
	}
	...
	protected static function ip_user_public_helper( string $ips ) : string {
		...
		foreach ( $ips as $_ip ) {
			if ( U\Str::is_user_public_ip( $_ip ) ) {
				return U\Str::normalize_ip( $_ip );
			}
		}
		return ''; // Nothing.
	}
