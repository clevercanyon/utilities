 ...
 * @see   U\Dir
 */
trait Sys_Data_Members {
	/**
	 * Gets system data directory.
	 *
	 * @since 2021-12-15
	 *
	 ...
	 *
	 * @return string System data directory.
	 *
	 * @throws U\Fatal_Exception On failure to locate.
	 *
	 * @see   U\Dir::sys_dir_helper()
	 * @see   U\Dir::sys_data_generator()
	 */
	public static function sys_data( /*string|null */ ?string $nsc_fqn = null ) : string {
		$this_fqn_crux  = U\Pkg::fqn_crux( __METHOD__ );
		$namespace_crux = U\Pkg::namespace_crux( $nsc_fqn );

		$basename                = 'data'; // Systems directory basename.
		...
		foreach ( U\Dir::sys_data_generator() as $_base_dir ) {
	...
	/**
	 * System base data directory generator.
	 *
	 * @since 2022-02-03
	 *
	 * @return \Generator|string[]|array[] Potential base directories for a system data sub-directory.
	 *                                     To clarify further. Directories returned by this generator will serve
	 *                                     as a potential base directory for a system data sub-directory
	 ...
	 */
	protected static function sys_data_generator() : \Generator {
		...
		if ( U\Env::is_mac() ) {
			if ( $home_dir ??= U\Env::var( 'HOME' ) ) {
				yield U\Dir::join( $home_dir, '/Library/Application Support' );

			} elseif ( $user ??= U\Env::var( 'USER' ) ) {
				yield '/Users/' . $user . '/Library/Application Support';
			}
		...
		if ( U\Env::is_unix_based() || U\Env::is_unknown_os() ) {
			 ...
			 */
			if ( $_this_dir = U\Env::var( 'XDG_DATA_HOME' ) ) {
				yield [ $_this_dir, 'autocreate' => true ]; // XDG location.
			}
			if ( ( $home_dir ??= U\Env::var( 'HOME' ) ) && is_dir( $home_dir ) ) {
				yield [ U\Dir::join( $home_dir, '/.local/share' ), 'autocreate' => true ];

			} elseif ( ( $user ??= U\Env::var( 'USER' ) ) && is_dir( '/home/' . $user ) ) {
				yield [ '/home/' . $user . '/.local/share', 'autocreate' => true ];
			}
			if ( $_this_dirs = U\Env::var( 'XDG_DATA_DIRS' ) ) {
