 ...
 * @see   U\Dir
 */
trait Sys_State_Members {
	/**
	 * Gets system state directory.
	 *
	 * @since 2021-12-15
	 *
	 ...
	 *
	 * @return string System state directory.
	 *
	 * @throws U\Fatal_Exception On failure to locate.
	 *
	 * @see   U\Dir::sys_dir_helper()
	 * @see   U\Dir::sys_state_generator()
	 */
	public static function sys_state( /*string|null */ ?string $nsc_fqn = null ) : string {
		$this_fqn_crux  = U\Pkg::fqn_crux( __METHOD__ );
		$namespace_crux = U\Pkg::namespace_crux( $nsc_fqn );

		$basename                = 'state'; // Systems directory basename.
		...
		foreach ( U\Dir::sys_state_generator() as $_base_dir ) {
	...
	/**
	 * System base state directory generator.
	 *
	 * @since 2022-02-03
	 *
	 * @return \Generator|string[]|array[] Potential base directories for a system state sub-directory.
	 *                                     To clarify further. Directories returned by this generator will serve
	 *                                     as a potential base directory for a system state sub-directory
	...
	protected static function sys_state_generator() : \Generator {
		...
		if ( U\Env::is_unix_based() || U\Env::is_unknown_os() ) {
			 ...
			 */
			if ( $_this_dir = U\Env::var( 'XDG_STATE_HOME' ) ) {
				yield [ $_this_dir, 'autocreate' => true ]; // XDG location.
			}
			if ( ( $home_dir ??= U\Env::var( 'HOME' ) ) && is_dir( $home_dir ) ) {
				yield [ U\Dir::join( $home_dir, '/.local/state' ), 'autocreate' => true ];

			} elseif ( ( $user ??= U\Env::var( 'USER' ) ) && is_dir( '/home/' . $user ) ) {
				yield [ '/home/' . $user . '/.local/state', 'autocreate' => true ];
			}
			yield '/usr/local/var/lib';
			yield '/usr/local/var';
		}
	}
}
