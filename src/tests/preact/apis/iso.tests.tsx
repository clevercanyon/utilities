...
describe('$preactꓺiso tests', async () => {
	...
	test('$preactꓺiso.prerenderSPA()', async () => {
		...
		const othersOtherA1Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherA2Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherB1Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherB2Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherC1Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherC2Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherC3Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherD1Markup = await $preactꓺiso.prerenderSPA({
			request: new Request(new URL('http://x.tld/others/other-d?a=_a&b=_b&c=_c')),
			appManifest: { 'index.html': { css: ['style.css'], file: 'script.js' } },
			App, // Defined above.
		});
		expect(othersOtherD1Markup).toContain('<!DOCTYPE html>');
		expect(othersOtherD1Markup).toContain('<title>404 Error: Not Found</title>');
		expect(othersOtherD1Markup).toContain('</html>');

		// ---

		const othersOtherD2Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherD3Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherE1Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherE2Markup = await $preactꓺiso.prerenderSPA({
		...
		const othersOtherE3Markup = await $preactꓺiso.prerenderSPA({
		...
		expect(othersOtherE3Markup).toContain('<title>404 Error: Not Found</title>');
