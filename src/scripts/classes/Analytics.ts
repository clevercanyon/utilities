...
export default class uAnalytics extends uA6tBase {
	 ...
	 */
	public async geoData() : Promise<uAnalyticsGeoData> {
		if ( this.cache.geoData ) {
			return this.cache.geoData;
		}
		return fetch( 'https://wobots.com/api/ip-geo/v1' )
			.then( response => response.json() )
			.then( json => this.cache.geoData = json as uAnalyticsGeoData );
	}
	...
	protected initialize() : void {
		...
		uDOM.on( 'click', 'a, button, input[type="button"], input[type="submit"]', event => void this.trackClick( event ) );
	}
